...
dim(penguins) #Numero de observaciones
colnames(penguins) #nombre de las variables
str(penguins) #tipo de variable
anyNA(penguins) #datos perdidos

# Estadistica descriptiva
summary(penguins) #Minimo, primer cuartil, mediana, media, tercer cuartil y maximo
...
color=c("cornflowerblue","blueviolet")
...
# 1.- Creación de un vector de color
color=c("darkseagreen", "gold", "chocolate")
...
HG<-ggplot(penguins, aes(x=largo_aleta_mm))+
  geom_histogram(col="darkorange4", fill="darkturquoise")+
  ggtitle("Histograma")+
...
GD2<-ggplot(penguins, aes(x=largo_pico_mm, y=grosor_pico_mm))+
  geom_point(aes(color=especie))+
  scale_color_manual(values=c('gold','darkseagreen', 'burlywood'))+
  ggtitle("Gráfico de dispersión")+
  xlab("largo del pico (mm)")+
  ylab("grosor del pico (mm)")+
  theme_light()

# Visualizacion del objeto 2
GD2
#-------------------------------
